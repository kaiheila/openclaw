...
> ⚠️ **法律免责声明（请先阅读）**
>
> 使用本指南配置并运行机器人，视为你已理解并同意：机器人权限、白名单和功能开关由你自行设置并承担风险。
>
> 因配置不当、账号泄露、权限过宽或其他操作失误造成的未授权调用、数据删除或系统损失（包括但不限于删除所在部署机器上的文件，自行修改配置文件造成权限异常等），责任由使用者自行承担。
>
> 在法律允许的最大范围内，KOOK 平台软件及其相关方不对因机器人配置或使用引发的直接或间接损失承担责任。
>
> 因此最好在虚拟机环境下进行本教程的部署
> **本文档适合：**
